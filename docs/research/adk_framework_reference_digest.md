...
| Instruction templating | A graph agent node needs none — the input model arrives as JSON user content. A bare `{key}` reads session state; a dotted `{Model.field}` fails the state-name check and reaches the model as literal braces `[L1.28]` |
| Does a bare `{my_key}` work? | **Yes — it reads session state.** `_is_valid_state_name` accepts any Python identifier, and one behind `app:`/`user:`/`temp:`. The published docs imply otherwise; the source says this. What does **not** work is a dotted name `[L1.28]` |

**What this changed.** The plan had `asset_generator_agent`'s instruction reading `{style_rules}` as a bare state key. The first correction replaced it with `{AssetGenInput.style_rules}` on the strength of the published docs — and that form is **worse**, because the dot fails the state-name check and the braces reach the model as literal text with nothing raised. Fix: a small **code node**, `attach_style_rules`, sits between extraction and generation, reads `ACTIVE` rules live, and returns an `AssetGenInput` model. The generator then uses **no template**: a graph agent node receives its input model as JSON user content, so the rules are already in the prompt.
